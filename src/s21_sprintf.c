...
int s21_sprintf(char *str, const char *format, ...) {
  va_list var; //Создаём указатель на аргументы типа va_list, который будет итерироваться по аргументам ...
  va_start(var, format); //Данная функция ствыить указатель var на первый аргумент идущий после последнего фиксированного аргумента то есть после format
  char *strStart = str; //Фиксируем адресс начала строки в, которую будем записывать результат функции sprintf()
  while (*format) { //Бежим по форматной строке
    flags flag = {0}; //Создаём переменнуб структурного типа flags и инициализируем нолями
    if (*format != '%') { //Пока не встретили симовол % бежим по строке и тупо переписываем её
      *str++ = *format++;
    } else { //Как только встречается символ %
      format++; //Перемещаемся на следуюжий элемент
      format = parse_Format(format, &flag, var); //парсим флаги
      str = specifier(str, &flag, var);
      if (flag.specifier == 'n') { //Тут всё просто записываем колличество считанных элементов по адресу взятому из аргументов
        int *pointer = va_arg(var, int *); //Функция va_arg() получает из аргументов текущий аргумент по указателю var, но при этом нужно указать тип получаемого аргумента в нашем случае int*
        *pointer = str - strStart;
      }
    }
  }
  va_end(var); //Очищаю переменную var
  return str - strStart; //Возвращаем кол-во Записаных элементов
}

const char *parse_Format(const char *format, flags *f, va_list var) {
  format = parseFlags(format, f);
  format = parse_Width(format, f, var); //тут добавляем var так как может встретиться * и тогда значение надо будет считать из аргументов
  format = parsePrecision(format, f, var);  //тут добавляем var так как может встретиться * и тогда значение надо будет считать из аргументов
...
const char *parse_Width(const char *format, flags *f, va_list var) {
  if (*format == '*') {
    format++;
    f->width = va_arg(var, int); //Лиобо записываем из аргументов если *
  } else if (*format >= 48 && *format <= 57) { //Либо записываем цыфровые чары
    char tempWidth[BUFFER_SIZE] = "";
    for (int i = 0; *format >= 48 && *format <= 57; i++, format++) { //Бежим по строке и записываем цыфровые чары
      tempWidth[i] = *format;
    }
    f->width = s21_atoisprint(tempWidth); //Переводим строку цыфр в число 
  }
  return format;
}
